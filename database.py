#!/usr/bin/env python3
"""
Database module for Telegram Parser Bot
Optimized for SQLite with aiosqlite
"""
...
class Database:
    ...
    async def get_user_stats_extended(self, user_id: int) -> Dict:
        ...
        cursor = await self.conn.execute('''
            SELECT 
                ...
                SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END) as completed_sessions,
                COALESCE(SUM(parsed_items), 0) as total_members,
                COALESCE(AVG(parsed_items), 0) as avg_per_session,
        ...
        return {
            ...
            'completed_sessions': completed,
            'total_members': row['total_members'] or 0,
            'avg_per_session': row['avg_per_session'] or 0,
            'success_rate': (completed / total * 100) if total > 0 else 0,
            'last_session': row['last_session_date']
        }
    
    async def get_all_users_with_stats(self, limit: int = 10, offset: int = 0) -> List[Dict]:
        """Получить пользователей со статистикой"""
        cursor = await self.conn.execute('''
            SELECT 
                u.*,
                (SELECT COUNT(*) FROM parsing_sessions WHERE user_id = u.user_id) as total_sessions,
                (SELECT COUNT(*) FROM parsing_sessions WHERE user_id = u.user_id AND status = 'completed') as completed_sessions,
                (SELECT COALESCE(SUM(parsed_items), 0) FROM parsing_sessions WHERE user_id = u.user_id) as total_members,
                (SELECT COUNT(*) FROM subscriptions WHERE user_id = u.user_id AND status = 'active' AND expires_at > CURRENT_TIMESTAMP) as has_active_sub
            FROM users u
            ORDER BY u.created_at DESC
            LIMIT ? OFFSET ?
        ''', (limit, offset))
    ...
    async def get_daily_stats(self, days: int = 30) -> Dict:
        ...
        cursor = await self.conn.execute('''
            SELECT 
                ...
                SUM(parsing_count) as daily_parsings,
                SUM(members_parsed) as daily_members
            FROM usage_stats
            WHERE date >= date('now', ?)
            GROUP BY date
            ORDER BY date DESC
        ...
        return {
            ...
            'total_users': len(set(row['user_id'] for row in rows if row['user_id'])),
            'total_parsings': sum(row['daily_parsings'] or 0 for row in rows),
            'total_members': sum(row['daily_members'] or 0 for row in rows)
        }
    
    async def get_total_revenue(self) -> float:
        """Общий доход"""
        cursor = await self.conn.execute('''
            SELECT COALESCE(SUM(price), 0) as total FROM subscriptions WHERE price > 0
        ''')
    ...
    async def add_test_data(self, user_id: int, count: int = 10):
        """Добавить тестовые данные парсинга"""
        for i in range(count):
            session_uid = str(uuid.uuid4())
            await self.conn.execute('''
                INSERT INTO parsing_sessions 
                (user_id, session_uid, channel_url, parsing_type, status, parsed_items)
                VALUES (?, ?, ?, 'members', 'completed', ?)
            ''', (user_id, session_uid, f'test_channel_{i}', random.randint(10, 100)))
        
        await self.conn.commit()
        logger.info(f"Добавлено {count} тестовых сессий для пользователя {user_id}")
